...
//                         ->get();
//
//         $userIds = $events->pluck('event_maker')->map(fn($id) => (int) $id)->unique()->values()->all();
// dd($events);
// //         $usersResponse = Http::post('http://localhost:8005/api/user/getUsersByIds', [
// //                 'ids' => $userIds
// //         ]);
// //
...
//
//         $formattedEvents = $events->map(function ($event) use ($usersMap) {
//                 $eventData = (new EventResource($event))->toArray(request());
//
//                 $eventData['event_maker'] = $usersMap[$event->event_maker] ?? null;
//
//                 return $eventData;
//             });
//
...
public function getEvents(Request $request)
    {
        ...
        $usersResponse = Http::get('http://localhost:8005/api/user/getUsersByIds', [
                'ids' => $userIds
         ]);

        $users = $usersResponse->json();

        $usersMap = collect($users)->mapWithKeys(fn($user) => [$user['properties']['id'] => $user]);

        $formattedEvents = $events->map(function ($event) use ($usersMap) {
                $eventData = (new EventResource($event))->toArray(request());

                $eventData['event_maker'] = $usersMap->get($event->event_maker) ?? null;
